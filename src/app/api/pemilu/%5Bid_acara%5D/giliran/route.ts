...
interface RouteContext {
  params: { id: string }; // dari folder [id]
}

export async function POST(request: NextRequest, context: RouteContext) {
  try {
    const { id } = context.params; // ← ambil dari URL /api/pemilu/[id]/join
    ...
    const nextOrder = lastPeserta ? lastPeserta.nomor_urut + 1 : 1;
    ...
    const newPeserta = await prisma.pesertaAcara.create({
        data: {
          ...
          // status_peserta tidak di-set → pakai default(TERDAFTAR)
        },
      });      
