    ...
    <div className="w-full relative">
      ...
      <div className="hidden lg:grid grid-cols-4 grid-rows-2 gap-3 h-[450px] rounded-2xl overflow-hidden">
        <div className="col-span-2 row-span-2 relative cursor-pointer group overflow-hidden" onClick={() => openLightbox(0)}>
          <Image src={displayImagesGrid[0]} alt="Main" fill className="object-cover transition-transform duration-700 group-hover:scale-110" priority/>
          ...
          <div key={idx} className="relative cursor-pointer group col-span-1 row-span-1 overflow-hidden" onClick={() => openLightbox(idx + 1)}>
            ...
            {idx === 3 && (
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                 <button className="bg-white text-black px-4 py-2 rounded-lg text-xs font-bold shadow-lg flex items-center gap-2"><Icon icon="solar:gallery-bold-duotone"/> Lihat Semua</button>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="lg:hidden relative h-[320px] w-full rounded-2xl overflow-hidden shadow-sm group">
         ...
         <div 
            ref={mobileSliderRef}
            onScroll={handleScrollUpdate} // <--- Deteksi swipe manual user
         ...
         {/* --- ARROW KIRI (HANYA MUNCUL JIKA BUKAN GAMBAR PERTAMA) --- */}
         {mobileActiveIndex > 0 && (
            <button 
         ...
         {/* --- ARROW KANAN (HANYA MUNCUL JIKA BUKAN GAMBAR TERAKHIR) --- */}
         ...
         {/* Badge Counter (Live Update sesuai swipe) */}
      ...
      {mounted && isOpen && createPortal(
        <AnimatePresence>
            ...
            >
                <button onClick={closeLightbox} className="absolute top-6 right-6 z-50 p-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition-all active:scale-95">
                ...
                >
                  <Image src={images[photoIndex]} alt="Cinema View" fill className="object-contain" priority quality={100}/>
                </motion.div>
