...
export function PageHeader({ tasks, totalDone }: { tasks: DailyTask[]; totalDone: number }) {
  ...
  return (
    ...
    <div className="-mx-4 sm:-mx-5 pt-0 pb-4">
      {/* Stack on mobile/tablet, switch to 3:1 grid only on large screens */}
      <div className="flex flex-col lg:grid lg:grid-cols-4 gap-3 lg:gap-4 items-stretch px-4 sm:px-5 pt-4">

        {/* ══════════════════════════════════
            BANNER  col-span-3
        ══════════════════════════════════ */}
        <div
          className="lg:col-span-3 relative overflow-hidden rounded-2xl min-h-[150px] sm:min-h-[156px] flex items-center"
          style={{
        ...
        >
          ...
          {/* ── content ── */}
          <div className="relative z-10 flex items-center justify-between w-full px-5 py-5 sm:px-8 sm:py-6 gap-4 sm:gap-6">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2.5 mb-2.5 sm:mb-3">
                ...
                <span className="text-[8.5px] sm:text-[9px] font-black uppercase tracking-[0.2em] sm:tracking-[0.34em] text-orange-400/80 select-none whitespace-nowrap">
              ...
              {/* Headline */}
              <h1
                className="text-[1.55rem] sm:text-[1.85rem] font-black tracking-tight leading-[1.08] mb-2 bg-clip-text text-transparent"
              ...
              {/* Subtitle */}
              <p className="text-[12px] sm:text-[12.5px] text-slate-400 leading-relaxed">
                Halo,{" "}
              ...
              <button
                ...
                className="mt-4 group inline-flex w-full sm:w-auto items-center justify-center sm:justify-start gap-2 rounded-xl px-4 py-2.5 text-[12px] font-bold text-white transition-all duration-200 select-none"
                style={{
            ...
            {/* Right: illustration — hidden on phones to avoid overlap, shown from sm */}
            <div className="hidden sm:block relative shrink-0 self-end select-none pointer-events-none">
              ...
              <Image
                ...
                width={220}
                height={160}
                className="relative h-[128px] lg:h-[160px] w-auto object-contain"
                style={{
                  filter:
        ...
        ══════════════════════════════════ */}
        <div
          className="lg:col-span-1 relative overflow-hidden rounded-2xl flex flex-row items-center gap-5 lg:flex-col lg:justify-center lg:gap-3.5 px-5 py-5 lg:py-6"
          style={{
        ...
        >
          ...
          {/* Label — top on desktop */}
          <p className="hidden lg:block text-[8px] font-black uppercase tracking-[0.28em] text-slate-600 w-full text-center">
            Progress Hari Ini
          </p>

          {/* Animated ring */}
          <div className="relative h-[84px] w-[84px] lg:h-[90px] lg:w-[90px] shrink-0">
            <div
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-[1.25rem] lg:text-[1.35rem] font-black text-white tabular-nums leading-none">
                {Math.round(animPct)}%
              </span>
          ...
          {/* Info column — beside ring on mobile, stacked below on desktop */}
          <div className="flex min-w-0 flex-1 flex-col items-start gap-2 lg:w-full lg:flex-none lg:items-center lg:gap-3.5">
            {/* Label — inline on mobile only */}
            <p className="lg:hidden text-[8px] font-black uppercase tracking-[0.28em] text-slate-600">
              Progress Hari Ini
            </p>

            {/* Count */}
            <div className="text-left lg:text-center">
              <p className="text-[1.6rem] lg:text-[1.75rem] font-black tabular-nums leading-none" style={{ color: ringColor }}>
                {totalDone}
